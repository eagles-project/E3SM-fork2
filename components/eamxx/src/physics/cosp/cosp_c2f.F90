...
!   cospstateIN%hgt_matrix_half(:,1:Nlevels) = zlev_half(start_idx:end_idx,Nlevels:1:-1) ! km
    ...
    modis_taut(:npoints)   = cospOUT%modis_Optical_Thickness_Total_Mean(:npoints)
    modis_tauw(:npoints)   = cospOUT%modis_Optical_Thickness_Water_Mean(:npoints)
    modis_taui(:npoints)   = cospOUT%modis_Optical_Thickness_Ice_Mean(:npoints)
    modis_reffw(:npoints)  = cospOUT%modis_Cloud_Particle_Size_Water_Mean(:npoints)
    modis_reffi(:npoints)  = cospOUT%modis_Cloud_Particle_Size_Ice_Mean(:npoints)
    modis_lwp(:npoints)    = cospOUT%modis_Liquid_Water_Path_Mean(:npoints)
    modis_iwp(:npoints)    = cospOUT%modis_Ice_Water_Path_Mean(:npoints)

    modis_cld_Q06(:npoints)   = cospOUT%modis_Cloud_Fraction_Nd_Q06_Mean(:npoints)
    modis_nd_Q06(:npoints)    = cospOUT%modis_Cloud_Top_Nd_Q06_Total_Mean(:npoints)
    modis_lwp_Q06(:npoints)   = cospOUT%modis_Cloud_Top_LWP_Q06_Total_Mean(:npoints)
    modis_tau_Q06(:npoints)   = cospOUT%modis_Cloud_Top_Tau_Q06_Total_Mean(:npoints)
    modis_reff_Q06(:npoints)  = cospOUT%modis_Cloud_Top_Size_Q06_Total_Mean(:npoints)

    modis_cld_ALL(:npoints)   = cospOUT%modis_Cloud_Fraction_Nd_ALL_Mean(:npoints)
    modis_nd_ALL(:npoints)    = cospOUT%modis_Cloud_Top_Nd_ALL_Total_Mean(:npoints)
    modis_lwp_ALL(:npoints)   = cospOUT%modis_Cloud_Top_LWP_ALL_Total_Mean(:npoints)
    modis_tau_ALL(:npoints)   = cospOUT%modis_Cloud_Top_Tau_ALL_Total_Mean(:npoints)
    modis_reff_ALL(:npoints)  = cospOUT%modis_Cloud_Top_Size_ALL_Total_Mean(:npoints)
    ! YQIN <<< 
